...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  /*quasi_heapを定義
  テストグラフ数*ヒープの長さの行列を定義
  各要素は構造体Counterを持つ
  */
  cout << "make quasi" << endl;
  Counter quasi_heap[test_gids.size()+train_gids.size()][m];
  StreamHeap streamheap[test_gids.size()+train_gids.size()];
  ...
  for(auto& group : groups){
    ...
    while (group_copy.size() > 0) {
      ...
      // ssqAlgorithm(quasi_heap,streamheap,"aaaaa", 0);
  ...
  for (uint32_t i = 0; i < num_intervals; i++) {
    ...
    for (uint32_t j = 0; j < a.size(); j++) {
      for(uint32_t k=0;k<test_gids.size();k++){//テスト結果だけほしい、トレの結果いらん
        if(j==test_gids[k]){
          cout<<a[j]<<" ";
        }
      }
